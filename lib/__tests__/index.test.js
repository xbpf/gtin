...
test.each([
  ...
  ['1234123412344'],
  ['12341234123413']
])('gtin.validate(%p) (valid)', barcode => {
  expect(gtin.validate(barcode)).toBe(true)
})
...
test.each([
  ...
  ['123412341234', 4],
  ['1234123412341', 3]
])('gtin.validate(%p) (invalid)', (partialBarcode, lastDigit) => {
  expect.assertions(9)
  for (let i = 0; i < 10; i++) {
    if (i === lastDigit) continue
    expect(gtin.validate(partialBarcode + i)).toBe(false)
  }
})
...
])('gtin.validate(%p) (error: %p)', (barcode, expectedError) => {
  expect(() => gtin.validate(barcode)).toThrow(expectedError)
})
